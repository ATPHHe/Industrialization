...
CAutoMinerSystem = CGlobalObjectSystem:derive("CAutoMinerSystem")

function CAutoMinerSystem:new()
	local o = CGlobalObjectSystem.new(self, "autominer")
	return o
end

function CAutoMinerSystem:isValidIsoObject(isoObject)
	return instanceof(isoObject, "IsoThumpable") and isoObject:getName() == "Industrialization Small Auto Miner"
end

function CAutoMinerSystem:newLuaObject(isoObject)
	return CAutoMinerGlobalObject:new(self, isoObject)
end

CGlobalObjectSystem.RegisterSystemClass(CAutoMinerSystem)
...
local function DoSpecialTooltip(tooltipUI, square)
	...
	--[[local barrel = CAutoMinerSystem.instance:getIsoObjectOnSquare(square)
